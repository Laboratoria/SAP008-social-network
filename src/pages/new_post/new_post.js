...
export default () => {
    ...
    const evaluationIcon = container.querySelectorAll('.circle-icon');
    // const rating = '';

    document.addEventListener('click', (e) => {
        const classEvaluationIcon = e.target.classList;
        if (!classEvaluationIcon.contains('active')) {
            evaluationIcon.forEach((evaluationIcon) => {
                evaluationIcon.classList.remove('active');
            });
            classEvaluationIcon.add('active');
            const foodRate = (e.target.getAttribute('rate-food'));
            const priceRate = (e.target.getAttribute('rate-price'));
            const placeRate = (e.target.getAttribute('rate-place'));
//            const percentageRate = (foodRate + priceRate + placeRate) / 3;
